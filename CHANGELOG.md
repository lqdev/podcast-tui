...
- **Keybinding conflict detection** — `KeyHandler::validate()` detects duplicate chord assignments and unbound critical actions (Quit, ShowHelp, Cancel). Conflicts are logged as warnings to stderr at startup; a missing Quit binding aborts startup with a clear error. Closes [#99](https://github.com/lqdev/podcast-tui/issues/99).
