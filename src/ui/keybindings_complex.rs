...
impl KeyHandler {
    ...
    fn setup_default_bindings(&mut self) {
        // Simple, clash-free keybindings for maximum compatibility
        
        // Basic navigation - these should work everywhere
        ...
        // Cancel/quit - simple and universal
        ...
        // Function keys - these almost never clash
        ...
        self.bind_key(KeyChord::none(KeyCode::F(10)), UIAction::Quit); // F10 is rarely used

        // Tab navigation - usually works
        ...
        // Content interaction - basic keys
        ...
        // VS Code-friendly buffer switching alternatives
        self.bind_key(KeyChord::alt(KeyCode::Char('b')), UIAction::ShowMinibuffer("Switch buffer: ".to_string()));
        self.bind_key(KeyChord::ctrl(KeyCode::Char('u')), UIAction::ShowMinibuffer("Switch buffer: ".to_string()));

        // Buffer management (C-x prefix) with alternatives
        self.bind_sequence(
            KeySequence::from_chord_and_key(
                KeyChord::ctrl(KeyCode::Char('x')),
        ...
        // VS Code-friendly quit alternatives
        self.bind_key(KeyChord::ctrl(KeyCode::Char('q')), UIAction::Quit);
        self.bind_key(KeyChord::alt(KeyCode::F(4)), UIAction::Quit);
        ...
        // Command execution (with VS Code-friendly alternatives)
        self.bind_key(KeyChord::alt(KeyCode::Char('x')), UIAction::PromptCommand);
        self.bind_key(KeyChord::ctrl(KeyCode::Char(':')), UIAction::PromptCommand); // Vi-style alternative

        // Content interaction
        self.bind_key(KeyChord::none(KeyCode::Enter), UIAction::SelectItem);
        self.bind_key(KeyChord::none(KeyCode::Char(' ')), UIAction::SelectItem);

        // Refresh (with VS Code-friendly alternatives)
        ...
        // Podcast management (with more alternatives)
        self.bind_key(KeyChord::none(KeyCode::Char('a')), UIAction::AddPodcast);
        self.bind_key(KeyChord::none(KeyCode::Char('d')), UIAction::DeletePodcast);
        self.bind_key(KeyChord::none(KeyCode::Char('r')), UIAction::RefreshPodcast);
        self.bind_key(KeyChord::shift(KeyCode::Char('R')), UIAction::RefreshAll);
        
        // Alternative podcast management keybindings
        self.bind_key(KeyChord::alt(KeyCode::Char('a')), UIAction::AddPodcast);
        self.bind_key(KeyChord::ctrl(KeyCode::Char('r')), UIAction::RefreshPodcast);

        // Quick buffer switching (F-keys work well in VS Code)
        self.bind_key(KeyChord::none(KeyCode::F(2)), UIAction::SwitchBuffer("podcast-list".to_string()));
        self.bind_key(KeyChord::none(KeyCode::F(3)), UIAction::SwitchBuffer("*Help*".to_string()));
        
        // Additional VS Code-friendly shortcuts
        self.bind_key(KeyChord::ctrl(KeyCode::Char('h')), UIAction::ShowHelp); // Alternative to C-h
        self.bind_key(KeyChord::alt(KeyCode::Char('h')), UIAction::ShowHelp);
    }
