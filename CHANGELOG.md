...
- **Runtime device profile switching** — New `:set-device-profile [name]` minibuffer command switches the active device profile without restarting the app. Tab-completes against the names in `config.device_profiles`. An empty argument clears the active profile. Unknown names show an error and leave state unchanged. The Sync buffer header updates immediately. Limitation: the change is **in-memory only** — it does not write back to `config.json`, so it does not survive restart (matches existing behavior of the `theme` command; full persistence tracked in [#223](https://github.com/lqdev/podcast-tui/issues/223)). Closes #210.
