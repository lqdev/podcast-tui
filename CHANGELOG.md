...
- **Body-hash short-circuit on unchanged feeds (Layer 2 of incremental refresh)** — Layer 1 (#246) only helps when servers honor RFC 7232 conditional GET. Many self-hosted feed hosts don't (no `ETag`, no `Last-Modified`, or they ignore `If-None-Match`/`If-Modified-Since`) and always return a full `200` body. `SubscriptionManager::refresh_feed` now SHA-256-hashes every successful response body, persists the hash on the `Podcast` (`last_body_hash` — schema-additive, defaults to `None`), and on the next refresh skips parse + dedup + renumber + per-episode save when the new hash matches the stored one. Catches the long tail of non-conformant hosts at a cost of ~1ms per refresh. Layer 1's 304 path takes precedence (no body to hash). Hard refresh (`C-r`) and the very first refresh after subscribe always fall through to the full path. New cache validators from the response (e.g., a fresh `ETag` accompanying an identical body) are still persisted on the short-circuit path so subsequent refreshes can use Layer 1. Eight tests added: 3 unit tests against `hash_feed_body` (determinism, sensitivity, lowercase-hex format) and 5 integration tests against `SubscriptionManager` + `JsonStorage` (first refresh stores hash, identical body short-circuits, changed body re-processes, hard refresh bypasses, new ETag persists across short-circuit). Closes #247.
- **Incremental refresh via HTTP conditional GET** — `SubscriptionManager::refresh_feed` now sends `If-None-Match` (ETag) and/or `If-Modified-Since` whenever the corresponding validator was captured on a previous 200 response, and persists whatever validators the server returns. First refreshes (and feeds whose servers never sent validators) continue to fetch unconditionally exactly as before. When the server responds `304 Not Modified` (the dominant case for unchanged feeds), the refresh short-circuits before parse/dedup/renumber/save — only `last_updated` is bumped — turning a typical ~1–3s of work per podcast into a ~50ms round-trip. Hard refresh (`C-r`) bypasses the cache by sending no conditional headers, preserving the existing "force fetch + reprocess everything" semantics. Schema-additive: existing `podcasts.json` files load unchanged (`last_etag`/`last_modified` default to `None`) and populate naturally on the first refresh after upgrade. Thirteen tests cover the wire-level header round-trip (10 unit tests against `FeedParser` using `wiremock`, including a regression that an unsolicited 304 from a misconfigured proxy is surfaced as an error rather than silently treated as "unchanged") and end-to-end refresh integration (4 tests against `SubscriptionManager` + `JsonStorage`, including preservation of stored validators when a 200 omits the headers). Closes #246.
...
- **Per-record save/delete is now serialized so the storage cache never disagrees with disk** — Follow-up to #230 (PR #233). With deterministic temp filenames replaced by UUID-suffixed temp paths, two concurrent saves of the same record could no longer corrupt the destination — but they could still finish in the opposite order on disk vs. in the in-memory cache. Concretely: writer A renames its temp file last (so disk reflects A), but writer B happens to update the cache last (so cache reflects B). Subsequent `load_*` calls served stale data until the next process restart. `JsonStorage` now keeps a per-id async-mutex map (`RecordLocks`) keyed by `podcast:<id>`, `episode:<podcast_id>:<episode_id>`, and `playlist:<id>`, and every `save_*` / `delete_*` takes the per-id guard for the entire (atomic-write + cache-update) critical section. Operations on distinct ids contend only for a brief HashMap lookup, so there is no measurable cost on the cold-load benchmark. Four regression tests cover concurrent same-target writers (32 podcasts, 32 episodes, 16 playlists) and concurrent save/delete on the same episode id, asserting that the cached value equals the on-disk value (or that both are absent) once everything settles. Closes #234.
- **Orphan UUID temp files from crashed writes are cleaned up on startup** — Follow-up to #230. The atomic-write hardening switched per-record saves to unique `<file>.<uuid>.tmp` temp paths, which removed the torn-write race but meant a crash mid-save could leave behind orphan temp files that no longer share a deterministic name with their target. `JsonStorage::initialize` now sweeps the data, podcasts, episodes, and playlists directories (one level deep) and removes any file matching the `<original>.<32-hex>.tmp` orphan pattern, logging the count if anything was reclaimed. The sweep (and the legacy `cache_index.json.tmp` cleanup) runs unconditionally — `atomic_write` produces orphans regardless of `storage.cache_enabled`, so gating cleanup on the cache flag would let orphans accumulate forever on cache-disabled installs. Cleanup is best-effort and never blocks startup. Legit `*.json` files and unrelated `.tmp` files (e.g. user backups) are left untouched. Two predicate tests and three integration tests cover the sweep (including a regression test for the cache-disabled path). Closes #235.
- **`episode_number` no longer balloons across refreshes (Y1 device-template fix)** — Refresh-time track numbering previously had two broken paths. The incremental path computed `max(existing_episode_numbers) + (new_index + 1)`, so every time the dedup loop misclassified a re-published episode as new, the running max snowballed; observed in the wild, a long-running show ended up with `episode_number` values in the hundreds of thousands against an actual episode count in the low thousands (orders of magnitude too large). The hard-refresh path renumbered correctly but the caller's dedup loop only persisted episodes flagged as new or updated, so the renumbered values for older episodes were silently discarded. Both bugs broke `{track}`-based device filename templates on alphabetically-sorting devices like the Innioasis Y1, since the field was both wildly inflated and inconsistent (many older episodes still had `episode_number: null`). The fix restructures `refresh_feed_with_options` to load existing episodes once, build a unified list of (existing + hard-refresh-updated + truly new) episodes, sort chronologically by `published` (with episode id as a deterministic tie-breaker so timestamp ties don't churn numbers across runs), assign dense `1..N` numbers, then save anything whose value actually changed. A new `migrate_episode_numbering` runs once at startup to detect and repair any podcast already in the broken state — cheap on healthy data (no writes), authoritative on broken data (full chronological renumber). Six unit tests cover the noop, snowballed, mixed-null, idempotency, empty-podcast, and aggregate-migration cases. Closes #231.
- **Atomic per-record saves no longer race on the temp file (catastrophic torn-write fix)** — `JsonStorage::atomic_write` previously used a deterministic temp path (`<file>.tmp`) for every save, so two concurrent saves of the same podcast or episode raced on the same temp file. The interleaved bytes were then renamed onto the destination, producing files that look like `<valid JSON>` followed by `]\n}` trailing junk — the classic torn-write signature. On startup `serde_json` choked on the trailing characters and the app refused to launch (`Error: Podcast data could not be read. Caused by: trailing characters at line N column M`), requiring users to hand-locate and repair every corrupt file. The temp filename now includes a random UUID (`<file>.<uuid>.tmp`) so concurrent writers never collide, and the temp file is best-effort cleaned up on rename failure to avoid leaks. The same hardening was applied to the persistent cache index flush. Three regression tests cover the long→short overwrite case, concurrent same-target podcast saves, and concurrent same-target episode saves. Closes #230.
...
- **Runtime config mutations persist to disk** — Mutations made by `:set-device-profile` and `:theme` are now written back to `config.json` (the file resolved by `--config` at startup, or the default platform-specific path) so they survive restart. Persistence is best-effort: if the save fails, an error is shown but the in-memory mutation still applies for the current session. Closes #223.
- **Customization & device profiles documentation** — New [`docs/DEVICE_PROFILES.md`](docs/DEVICE_PROFILES.md) end-to-end guide with an Innioasis Y1 headline example, schema reference, full token table, three worked profile examples (Innioasis Y1, generic flat, date-organised), runtime switching with `:set-device-profile`, preview-before-sync workflow, and troubleshooting (collisions, ASCII fold, length caps). README gains a `🎨 Customization` section pointing at startup buffer, keybinding presets, and device profiles. `GETTING_STARTED.md` Configuration section now documents `ui.startup_buffer` and links to the device profiles guide. `docs/ARCHITECTURE.md` documents the storage cache layer (in-memory + persistent `cache_index.json`, atomic flush, schema versioning, `:cache-rebuild` escape hatch). `docs/KEYBINDINGS.md` adds a Storage Commands section covering `:cache-rebuild`. Closes #211.
- **Storage cache benchmark** — New `examples/bench_storage_load.rs` quantifies the impact of the in-memory cache (#204) and persistent index (#205). On a 30-podcast × 200-episode synthetic fixture, warm-cache traversal is ~946× faster than the no-cache baseline (well above the 10× target from epic #202). Results and tuning notes documented in `docs/STORAGE_DESIGN.md`. Run with `cargo run --release --example bench_storage_load`. Closes #206.
- **Runtime device profile switching** — New `:set-device-profile [name]` minibuffer command switches the active device profile without restarting the app. Tab-completes against the names in `config.device_profiles`. An empty argument clears the active profile. Unknown names show an error and leave state unchanged. The Sync buffer header updates immediately. The change is also persisted to `config.json` so it survives restart (#223). Closes #210.
- **Sync uses active device profile** — When `active_device_profile` is set in `config.json`, `sync_to_device` now rewrites filenames inside the device's `Podcasts/` tree according to the profile's `filename_template`. The local downloads directory is never modified; only files copied to the device are renamed. Files outside `Podcasts/` (e.g. `Playlists/`) are forwarded verbatim. Collisions after templating are disambiguated by appending the last 6 chars of the episode ID. The Sync buffer status header now shows `Active device profile: <name>` (or `(none)`) so the active rule is always visible. Setting `active_device_profile = null` preserves byte-identical behavior to previous versions. Known limitation: `.m3u` playlists are not yet rewritten to point at the renamed device files (tracked as a follow-up). Closes #209.
- **Device filename template engine** — New `src/download/device_template.rs` module renders templated device-side filenames from a `(template, podcast, episode, ext, options)` tuple. Supports tokens `{podcast}`, `{podcast_short}` (truncated to 30 chars), `{title}`, `{track}`, `{track:NN}` (zero-padded), `{date}`, `{date:%fmt}` (chrono strftime), and `{ext}`. Literal `/` in the template creates subdirectories on the device while slashes inside substituted token values are sanitized away. Per-segment `max_length` cap (UTF-8-aware) and optional `ascii_only` post-processing. `disambiguate()` helper appends the last 6 chars of the episode ID for collision resolution. The cross-platform sanitizer (Windows reserved names, Unicode folding, length cap) was extracted from `download::manager` into a shared `download::sanitize` module so both consumers apply identical rules. Pure module — no I/O, no async. Sync wiring lands in #209. Closes #208.
- **Device profile config schema** — New `device_profiles` array and `active_device_profile` selector in `~/.config/podcast-tui/config.json` define per-device sync filename rules (template, max length, ASCII-only flag, structure preservation). Pure schema this release — the template engine and sync wiring land in #208 and #209. Existing `config.json` files load unchanged (both fields default to empty/none). Closes #207.
- **Persistent storage cache index** — The in-memory storage cache is now serialized to `cache_index.json` in the data directory and reloaded on startup, giving cold launches a warm cache. Writes are batched by a 5-second background flush task; a final flush runs on graceful shutdown to ensure no pending changes are lost. Schema-version mismatches and parse errors trigger an automatic rebuild from the source-of-truth JSON files. New `:cache-rebuild` minibuffer command provides a manual escape hatch (e.g., after editing files out-of-band) and reports `"Cache rebuilt: N podcasts, M episodes"`. Atomic writes via `.tmp + rename` prevent torn writes; stale `.tmp` files left from crashes are cleaned up on init. Closes #205.
...
- **Filename sanitizer no longer collapses ` - ` and ` _ ` separators** — `sanitize_filename` previously contained `.replace(" - ", "-")` and `.replace(" _ ", "_")` rules in its cleanup pass. Those rules fired inconsistently — a title like `Foo #123 - Bar` became `Foo number123-Bar` (collapsed) while `Foo: Bar` became `Foo- Bar` (not collapsed, because the `:` → `-` substitution lands earlier with no surrounding spaces). This silently rewrote user-supplied template separators too: a device template like `{date:%Y%m%d} - {title}.{ext}` rendered correctly initially, then the sanitizer mangled the templated dash, destroying the visual structure the user explicitly asked for. Both rules have been removed. The `--` and `__` collapses (which catch back-to-back substitutions like `:` → `-` next to an existing dash) are kept since they're typically not intentional. Title-internal punctuation and template separators now round-trip predictably to disk. Existing local downloads keep their existing names; only newly-downloaded files and freshly-synced device files use the new format. Six unit tests cover preserved spacing, multi-space collapse, double-dash/underscore preservation, and the now-unchanged `:` → `-` substitution path. Closes #232.
...
- **Eliminate redundant UIApp recreation during startup** — February 2026
  - `App::run()` previously discarded the fully-initialised `UIApp` built during the splash screen and rebuilt an empty one just to pass a new `app_event_tx`. A new `UIApp::set_app_event_tx()` method now wires the channel into the existing instance instead, eliminating the redundant reconstruction.
  - UIApp is now constructed exactly once; the pre-loaded buffer path in `UIApp::run()` is the path taken on every startup, and `initialize()` is no longer called on every run.
...
- **Defer download cleanup to background task on startup** — February 2026
  - `cleanup_stuck_downloads()` and `cleanup_old_downloads()` previously ran synchronously before the first UI render (in `initialize()`), blocking startup on slow disks or large libraries.
