...
- `set-device-profile [name]` — Switch the active device profile (in-memory). Tab-completes against profiles in `config.device_profiles`. Empty argument clears the active profile. **Limitation:** the change is not yet written back to `config.json`; it persists for the current session only.
