...
- **Playlist `.m3u` manifests now sync to the device** — Playlists copied to a device contained the audio files but no `.m3u` manifest, so media players couldn't open a synced playlist as a unit. The local manifests were generated correctly (#255) but the device-sync path never copied them: `scan_playlists_for_sync` only scanned each playlist's `audio/` subfolder, so the sibling manifest was never added to the sync set — and even a verbatim copy would have been broken, because device sync flattens playlists (`Playlists/<name>/audio/<file>` → `Playlists/<name>/<file>`, no `audio/` subfolder) while the local manifest references `audio/<file>`. `sync_to_device` now writes a device-layout manifest at `Playlists/<name>/<name>.m3u` for every playlist that has synced audio, rewriting each path line to drop the `audio/` prefix while preserving `#EXTM3U`/`#EXTINF` titles and episode order. Writes are idempotent (unchanged manifests are skipped), honored in dry-run (reported but not written), regenerated by hard sync, and — because `.m3u` files are excluded from the audio-only device scan — reconciled by a dedicated pass that removes a stale device manifest once its playlist no longer has synced audio. Closes #266.
...
- **Device sync reclaims stale and replaced files before copying** — Normal
  sync now deletes managed orphaned files, changed same-path replacements, and
  stale or changed playlist manifests before writing new content. This avoids
  temporary double-occupancy on space-constrained devices. Replacement cleanup
  still runs when orphan deletion is disabled, flat profiles continue to
  preserve unknown root-level files, and failed replacement deletions block only
  the affected copy while other sync operations continue.
