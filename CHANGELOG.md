...
- **NowPlaying buffer — real-time playback status display** — new `NowPlayingBuffer` implementing the `Buffer` trait that shows current episode title, podcast name, playback state icon (▶/⏸/⏹), progress bar, and volume level. Updates non-blocking from a `watch::Receiver<PlaybackStatus>` on each render tick. Gracefully handles stopped state (no episode playing), external player mode (position/duration unavailable), and small terminal areas (compact fallback rendering). Accessible via `F9` keybinding. Closes [#140](https://github.com/lqdev/podcast-tui/issues/140). Part of [#137](https://github.com/lqdev/podcast-tui/issues/137).
  - New `src/ui/buffers/now_playing.rs`: `NowPlayingBuffer` struct, `format_duration()` helper, `progress_ratio_and_label()` helper
