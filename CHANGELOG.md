...
- **Per-record save/delete is now serialized so the storage cache never disagrees with disk** — Follow-up to #230 (PR #236). With deterministic temp filenames replaced by UUID-suffixed temp paths, two concurrent saves of the same record could no longer corrupt the destination — but they could still finish in the opposite order on disk vs. in the in-memory cache. Concretely: writer A renames its temp file last (so disk reflects A), but writer B happens to update the cache last (so cache reflects B). Subsequent `load_*` calls served stale data until the next process restart. `JsonStorage` now keeps a sharded async-mutex map (`RecordLocks`) keyed by `podcast:<id>`, `episode:<podcast_id>:<episode_id>`, and `playlist:<id>`, and every `save_*` / `delete_*` takes the per-id guard for the entire (atomic-write + cache-update) critical section. Distinct ids do not contend, so there is no measurable cost on the cold-load benchmark. Three regression tests spawn 32 concurrent same-target writers (podcast, episode, playlist) and assert that the cached value byte-equals the on-disk value once everything settles. Closes #234.
