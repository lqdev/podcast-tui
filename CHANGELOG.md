...
- **`episode_number` no longer balloons across refreshes (Y1 device-template fix)** — `SubscriptionManager::assign_track_numbers` previously had two paths that both produced wrong results. The incremental refresh path computed `max(existing_episode_numbers) + (new_index + 1)`, so every time the dedup loop misclassified a re-published episode as new, `max_track` snowballed; one user's Joe Rogan Experience #2486 ended up with `episode_number: 168025` against ~2,500 real episodes (~67× too large). The hard-refresh path renumbered correctly but the caller's dedup loop only persisted episodes flagged as new or updated, so the renumbered values for older episodes were silently discarded. Both bugs broke `{track}`-based device filename templates on alphabetically-sorting devices like the Innioasis Y1, since the field was both wildly inflated and inconsistent (many older episodes still had `episode_number: null`). The fix restructures `refresh_feed_with_options` to load existing episodes once, build a unified list of (existing + hard-refresh-updated + truly new) episodes, sort by `published`, assign dense `1..N` numbers, then save anything whose value actually changed. A new `migrate_episode_numbering` runs once at startup to detect and repair any podcast already in the broken state — cheap on healthy data (no writes), authoritative on broken data (full chronological renumber). Six unit tests cover the noop, snowballed, mixed-null, idempotency, empty-podcast, and aggregate-migration cases. Closes #231.
